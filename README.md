...
- 채용공고(`Job`)와 채용공고기술(`JobSkill`)이 양방향이라 채용공고기술 수정 시 List 관리가 어려웠습니다. 
  먼저 작성한 코드에서는 직접 `JobSkillRepository`에 save하였으나 List 관리가 되지 않아,
  채용공고에 별도로 채용공고기술을 추가하는 메소드를 작성하고, cascade를 이용하여 저장되도록 했습니다.
- 채용공고 기술 수정 발생 시 mocking 데이터 만들기가 어려워서 통합 테스트로 대체하고, 레포지토리 테스트도 따로 수행하지 않았습니다. 
